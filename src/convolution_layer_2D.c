...
int optimised_layer_v3_x3m3_unroll_d16_FP(const float* in_FP, const float* filter_FP, const float* bias_array_FP, float* out_to_compare_with_FP) {
  ...
  printf("\n from optv3  %f %f ", out_to_compare_with_FP[0], out_to_compare_with_FP[1]);
  return 0;
}


// same as above, for x3m3 non-opt
// ~62 GFLOPS, better perf than above, making instruction cache miss perf more likely? profile with cachegrind/ likwid to find out more
...
// try 3x3 , m3, x3 - have to create fallback for loop to deal with any leftover iterations
// also maybe try 3x4/ 4x3?
